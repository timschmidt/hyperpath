...
fuzz_target!(|data: &[u8]| {
    ...
    let report =
        check_trace_orthogonal_pad_clearance(&trace, &pad, &clearance, PredicatePolicy::STRICT);
    ...
    let board_report =
        check_orthogonal_pad_board_clearance(&pad, &board, &clearance, PredicatePolicy::STRICT);
