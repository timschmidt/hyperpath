...
fn path_predicates(c: &mut Criterion) {
    ...
    let cam_exact_cutout = CamExactClipCutoutHandoff::new(
    ...
    .unwrap();
    c.bench_function("cam_exact_cutout_support_clip_program_replay", |b| {
        b.iter(|| {
            let report = build_cam_support_clip_program(
                support_plan.clone(),
                r(0),
    ...
    let simple_infill_clip_boundary = CamSupportClipBoundary::simple(
        vec![
            p(0, 0),
            p(10_000, 0),
            p(8_000, 1_000),
    ...
    .unwrap();
    c.bench_function("cam_simple_infill_clip_program_replay", |b| {
        b.iter(|| {
            let report = build_cam_infill_clip_program(
                clip_infill_graph.clone(),
                r(0),
                r(500),
                simple_infill_clip_boundary.clone(),
                PredicatePolicy::default(),
            )
            .unwrap();
            report.validate_replay(PredicatePolicy::default())
    ...
    let pcb_orthogonal_polygon = PcbOrthogonalPolyPad::new(
        NetId(7),
        TraceLayer(0),
        vec![
            p(11_000, 1_000),
    ...
    .unwrap();
    c.bench_function("pcb_orthogonal_polygon_copper_union_program_replay", |b| {
        b.iter(|| {
            let report = build_pcb_copper_union_program(
                vec![
                    PcbCopperBooleanSource::Trace(pcb_trace.clone()),
    ...
    c.bench_function("pcb_composite_copper_union_program_replay", |b| {
        b.iter(|| {
            let report = build_pcb_composite_copper_union_program(
                vec![
                    PcbCompositeCopperBooleanSource::HoledOrthogonal(pcb_holed.clone()),
                    PcbCompositeCopperBooleanSource::Solid(PcbCopperBooleanSource::Trace(
                        pcb_trace_second.clone(),
                    )),
                    PcbCompositeCopperBooleanSource::Solid(
                        PcbCopperBooleanSource::OrthogonalPolyPad(pcb_orthogonal_polygon.clone()),
                    ),
                ],
                pcb_z.clone(),
    ...
    let cam_rest_handoff = PathExactMeshHandoffSource::from_exact_mesh(
        rectangular_prism_from_i64_bounds(
            [2_000, 2_000, 0],
            [6_000, 5_000, 2_000],
            PredicatePolicy::default(),
    ...
    .unwrap();
    let cam_rest_handoff_cutter = CamRestMaterialCutter::exact_handoff(cam_rest_handoff).unwrap();
    c.bench_function("cam_exact_handoff_rest_material_program_replay", |b| {
        b.iter(|| {
            let report = build_cam_rest_material_program(
                cam_stock.clone(),
                r(0),
