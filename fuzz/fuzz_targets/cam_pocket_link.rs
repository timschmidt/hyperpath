...
fuzz_target!(|data: &[u8]| {
    ...
    let plan =
        build_rectangular_pocket_plan(pocket, r(1), r(stepover), max_rings, PredicatePolicy::default())
            .unwrap();
    ...
    match build_rectangular_pocket_link_graph(plan.clone(), PredicatePolicy::default()) {
        Ok(graph) => {
            ...
            for segment in &graph.ring_segments {
                assert_eq!(segment.ring_index, graph.plan.rings[segment.ring_index].index);
            }
        }
