...
/// Exact strictly convex polygonal PCB pad or copper zone.
///
/// This carrier covers chamfered pads and simple polygonal copper islands
/// without flattening curves or approximating arbitrary footprints. It reuses
/// the same strict-convexity predicate discipline as [`PcbConvexBoardOutline`]:
/// import/routing owns the retained vertices, while downstream mesh booleans
/// may lower them only after exact orientation and convexity are certified.
#[derive(Clone, Debug, PartialEq)]
pub struct PcbConvexPolyPad {
    net: NetId,
    layer: TraceLayer,
    vertices: Vec<Point2>,
    orientation: BoardContourOrientation,
...
/// Exact simple orthogonal PCB pad or copper zone.
///
/// This carrier covers nonconvex rectilinear copper while still staying inside
/// exact straight-edge topology. It shares validation with
/// [`PcbOrthogonalBoardOutline`]: every edge is horizontal or vertical, the
/// loop is simple, and holes remain future planar arrangement evidence.
#[derive(Clone, Debug, PartialEq)]
pub struct PcbOrthogonalPolyPad {
    net: NetId,
    layer: TraceLayer,
    vertices: Vec<Point2>,
    orientation: BoardContourOrientation,
...
impl PcbConvexPolyPad {
    /// Construct a strictly convex polygonal copper source.
    pub fn new(
        net: NetId,
        layer: TraceLayer,
        vertices: Vec<Point2>,
    ...
    /// Construct a strictly convex polygonal copper source with provenance.
    pub fn with_provenance(
        net: NetId,
        layer: TraceLayer,
        vertices: Vec<Point2>,
    ...
    /// Return certified polygon orientation.
    pub const fn orientation(&self) -> BoardContourOrientation {
        self.orientation
    }
...
impl PcbOrthogonalPolyPad {
    /// Construct a simple orthogonal polygonal copper source.
    pub fn new(
        net: NetId,
        layer: TraceLayer,
        vertices: Vec<Point2>,
    ...
    /// Construct a simple orthogonal polygonal copper source with provenance.
    pub fn with_provenance(
        net: NetId,
        layer: TraceLayer,
        vertices: Vec<Point2>,
